...
원하는 시나리오에 있는 JSON 파일을 다운로드 한 뒤 중괄호(`{}`)로 요구하는 데이터 전체를 입력하여여 사용할 수 있습니다. 특히 `[환경 설정]` 접두사가 붙은 데이터의 경우 **Pipeline > 환경 설정**에 있는 데이터를 사용하므로 데이터가 등록되어 있어야 합니다. [Pipeline 사용자 가이드](/Dev%20Tools/Pipeline/ko/environment-config/#_1)에서 등록 방법은 확인 가능합니다.
...
#### 소스 스테이지
소스 스테이지 중 Github을 기준으로 가이드가 작성되었습니다.
...
``` json
{
    ...
    "branch": "{배포 대상 브랜치}"  //배포 대상 소스 브랜치에 해당하는 값 입력이 필요합니다. (ex. main)
},
```
...
#### 빌드 스테이지
빌드 스테이지 중 NHN Cloud 빌드 도구를 기준으로 가이드가 작성되었습니다.
...
``` json
{
    ...
    "buildImageRegistry": "{[환경 설정]이미지 저장소 설정에 저장된 이미지 저장소 이름}",    //환경 설정에 등록한 이미지 저장소 이름 입력이 필요합니다. (ex. buildImageRegistry-pipeline)
    "buildImageName": "{이미지 이름}",  //이미지 이름 입력이 필요합니다. (ex. maven)
    "buildImageTag": "{이미지 태그}",   //이미지 태그 정보가 필요합니다. (ex. 1.0.0)
    "buildCommand": "{빌드 명령어}",    //빌드 명령어가 필요합니다. (ex. mvn package)
    "dockerfilePath": "{Dockerfile 경로}",  //Dockerfile의 경로 입력이 필요합니다. (ex. Dockerfile)
    "buildWorkDir": "",   //선택항목으로 Dockerfile의 실행 경로를 입력합니다.
    "pushImageRegistry": "{[환경 설정]빌드 결과가 업로드 될 이미지 저장소 설정에 저장된 이미지 저장소 이름}",  //환경 설정에 등록한 이미지 저장소 이름 입력이 필요합니다. (ex. pushImageRegistry-pipeline)
    "pushImageName": "{빌드 결과 이미지 이름}", //빌드 결과에 대한 이미지 이름 입력이 필요합니다.   (ex. result-image)
    "pushImageTag": "{빌드 결과 이미지 태그}",  //빌드 결과 이미지의 태그가 필요합니다. (ex. latest)
...
#### 배포 스테이지
배포 스테이지는 Deploy 스테이지로 가이드가 작성되었습니다.
...
``` json
{
    ...
    "deployTarget": "{[환경 설정]배포 대상 설정에 저장된 배포 대상 이름}",  //환경 설정에 등록한 배포 대상 이름 입력이 필요합니다. (ex. deploy-pipeline)
    "manifests": [
        // Deployment Manifest 정보 입력이 필요합니다.
        // 아래에 예시를 보시고 작성해서 등록하여 사용 가능합니다.
        {
            "apiVersion": "apps/v1",
            "kind": "Deployment",
            "metadata": {
...
Deploy Stage에서 사용되는 Manifest 정보의 경우 배포 환경에 맞도록 변경이 필요합니다.
yaml 파일을 json 형태로 변경이 필요합니다.(스테이지 변경을 통해 콘솔 UI에서 작업하시는 것을 추천드립니다.)
