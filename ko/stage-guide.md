...
- 배포를 실행하려는 아티팩트의 **Command Type**이 **SSH**인 경우 해당 **NHN Cloud Deploy 서비스 배포 실행** 기능을 지원을 하지 않으며, **Cloud Agenet**인 경우에만 지원을 합니다. 관련된 내용은 [Deploy 사용 가이드](https://docs.nhncloud.com/ko/Dev%20Tools/Deploy/ko/console-guide/#_8)를 참고하십시오.
...
**환경 설정**의 **NHN Cloud 보안 설정**에서 추가한 [NHN Cloud 보안 설정]을 선택하고, **AppKey**에는 사용할 NHN Cloud Deploy 서비스를 사용할 AppKey를 입력합니다.

입력 후 **검색** 버튼을 누르시면, 입력한 보안설정과 AppKey에 맞는 NHN Cloud Deploy의 배포 관련 정보들을 가져옵니다.
...
**배포 상세 설정**에서는, 배포할 대상들의 대한 조건을 추가할 수 있습니다.

**서버 선택**에는 배포할 서버들을 선택할 수 있습니다. **전체 서버** 클릭 시, 전체 서버를 대상으로 배포를 진행하며, **서버 선택** 클릭 시, 배포할 서버를 선택할 수 있습니다.

**동시 서버 실행 수**에는 NHN Cloud Deploy 서비스가 동시에 몇 개를 실행할 지 선택할 수 있습니다. (기본값 1, 최대 서버 개수).
